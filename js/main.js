...
let customScale = 1;
let targetFps = 60;
let bgmfile = './assets/audio/Theme_240.ogg';
let bgmvolume = 0;
let bgm;
...
const AUDIO_DETAIL = [
	{
		...
		dialog: {
			...
			English: [
				"Ngh... hmph! Nnnngh...",
				"Hah, it's on! It's... a tight fit, though."
			],
			Chinese: [
				"咕……嗯！嗯……",
				"哈，穿上了！勉强……"
	...
	},
	{
		...
		dialog: {
			Japanese: [
				"もう少し体を絞れば、ジャストサイズになります！"
			],
			English: [
				"If I just slim down a little more, it'll be a perfect fit!"
			],
			Chinese: [
				"再稍微瘦一点就刚刚好了！"
			]
	...
	},
	{
		...
		dialog: {
			...
			English: [
				"It's fine! I'm taking this swimsuit!"
			],
			Chinese: [
				"没关系！泳装就要这件了！"
			]
	...
	},
	{
		...
		dialog: {
			...
			English: [
				"On the contrary, this is what makes it so good...!"
			],
			Chinese: [
				"不如说，这样正好……！"
			]
...
const HITBOX = {
	headpat: { xMin: 2030, xMax: 2350, yMin: 40, yMax: 500 },
	voiceline: { xMin: 1000, xMax: 1760, yMin: 500, yMax: 1300 }
};
...
function releasedMouse() {
	switch (mouseSelect) {
		case 1:
			...
			spineData.state.setAnimation(2, 'PatEnd_02_M', false);
			spineData.state.addEmptyAnimation(1, 0.5, 0);
			spineData.state.addEmptyAnimation(2, 0.5, 0);
			break;
		case 2:
